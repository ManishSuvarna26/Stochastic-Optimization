...
nAvailableOperands = nVariableRegisters + nConstantRegisters; % R union C set (A)
constantRegistersSet = 1;
nOperators = 4; % 1=add, 2=substract, 3=multiply, 4 = divide
...
mutationVaryingFactor(1:nGenerations*rangeOfExploration) = linspace(5,0,nGenerations*rangeOfExploration);
for iGenerations = 1:nGenerations
    
for i = 1:populationSize
    chromosome = population(i).Chromosome;
    [fitness(i),totalError(i)] = EvaluateIndividual(chromosome,dataset,nVariableRegisters,constantRegistersSet,divisionProtection,maximumLengthOfChromosome);
